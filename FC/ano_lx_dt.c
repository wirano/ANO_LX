...
static void ANO_DT_LX_Data_Receive_Anl(uint8_t *data, uint8_t len)
{
    ...
    if (*(data + 2) == 0X20) {
        ...
        ano_leds.brightness[0] = *(data + 4);
        ano_leds.brightness[1] = *(data + 5);
        ano_leds.brightness[2] = *(data + 6);
        ano_leds.brightness[3] = *(data + 7);
    }
        //凌霄飞控当前的运行状态
    else if (*(data + 2) == 0X06) {
...
static void Add_Send_Data(uint8_t frame_num, uint8_t *_cnt, uint8_t send_buffer[])
{
    ...
    switch (frame_num) {
        ...
        case 0xe0: //CMD命令帧
        {
            ...
            break;
        case 0xa0: //LOG 信息输出--字符串
        {
            for (uint8_t i = 0; i < string_to_send.str_len; i++) {
                send_buffer[(*_cnt)++] = string_to_send.str[i];
            }
        }
            break;;
        case 0xe2: //PARA返回
