...
void Moving_Average(float moavarray[],//滤波数组 数组长度：len+1
                    uint16_t len,//滤波数据长度
                    uint16_t *fil_cnt,//滤波元素号数标记（静态，用作存储）
                    float in,//输入
                    float *out //输出
);
...
void fir_arrange_filter(float *arr, uint16_t len, uint8_t *fil_cnt, float in, float *arr_out);  //len<=255 len >= 3
...
float my_deadzone_3(float T, float hz, float x, float, float zoom, float range_x,
                    float *zoom_adj); //range_x   0 ----- 1  *****
